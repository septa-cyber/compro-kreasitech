...
const timelineData: TimelineItemProps[] = [
    {
        ...
        lineColorClass: "bg-gray-200 dark:bg-gray-700",
    ...
    },
    {
        ...
        lineColorClass: "bg-gray-300 dark:bg-gray-600",
        bgClass: "bg-background-gray dark:bg-background-dark-card",
    },
    {
        year: "2023",
        text: "Merambah ke area penyedia outsourcing untuk berbagai role talent IT ke beberapa client.",
        lineColorClass: "bg-purple-400",
        bgClass: "bg-deep-purple dark:bg-indigo-950 text-white",
    },
    {
        year: "2024",
            ...
            {timelineData.map((item, index) => (
                <section key={index} className={`py-20 ${item.bgClass}`}>
                    <div className="max-w-4xl mx-auto px-4 text-center">
                        <h3 className={`font-display text-3xl font-semibold mb-4 ${item.bgClass.includes('text-white') ? '' : 'text-gray-900 dark:text-white'}`}>
                            {item.year}
                        </h3>
                        <div className={`w-16 h-1 mx-auto mb-8 ${item.lineColorClass}`}></div>
                        <p className={`text-lg ${item.bgClass.includes('text-white') ? 'text-purple-100' : 'text-gray-600 dark:text-gray-400'} font-body`}>
                            {item.text}
                        </p>
                    </div>
                </section>
